...
contract has not made. The refusal message was generic rather than explaining
the whitened case. **Discharged (D8), in the smaller of the two forms the
debt allowed:** the refusal is now targeted rather than generic.
`.require_composition_bound_error_channel()` (`R/evidence-sampling.R`) fires
before the identity check whenever a whitened schedule meets a fit bound to the
native relation, and refuses with capability
`whitened_composition_sampling_law`, naming the composition as what unbound the
channel and `Σ̃ = Q^{1/2}ΣQ^{1/2}` as what a whitened calibration law would
need (`test-query-bank-sampling.R`, "a bank never reaches a learned metric or a
whitened plan"). **Still owed:** the whitened calibration law itself, which is
a new admission in `evidence-sampling-v1` and not a message.
...
| coherence spectrum | **delivered (D5):** `coherence_spectrum(x, weights, by_location = , by = , using = )` (`R/views.R`) aggregates a plan over a frame family, or the `effect_contrast_view` already read from one, into per-scale rows carrying `coherent`, `configuration`, `total` and the share φ_s computed from the **aggregated** components under the shared `.coherence_fraction()` mask. It is `contribution()`'s record and machinery under a scale-resolved grouping — same conservative-frame refusal, same masked `signed`, same `frame_relative` labelling — with the grouping generalized to a key *tuple*, because §11.4 G3 says the share lives on `(center, scale)`. `by_location = TRUE` returns that table; no location-wise collapse is offered, and `$metadata$aggregation$location_collapse` records `"none"`. `family` and `alpha` are carried into `$index` beside the energy they fix, and the provenance states which column is which: `alpha_fixed = "total"`, `alpha_invariant = "coherence_fraction"`. §3.1's prohibition is enforced rather than only documented — `plot(spectrum, which = "profile")` signals a `scale_energy_panel` refusal and an unqualified `plot()` draws the decomposition instead (`test-coherence-spectrum.R`) | met |
...
| latent PSD layer | **delivered (D7):** `latent_geometry(x, method = "psd_projection")` (`R/latent.R`) builds the §6 descriptive layer from a complete symmetric form or from the signed `geometry_spectrum()` view of one, and is the only place in the package that produces a nonnegative projection. `method` is the closed set G7 asks for — `"psd_projection"` (eigenvalue truncation at zero) built, `"nearest_psd"` declared and refusing rather than substituting. The sealed `effect_latent_geometry` carries the projected spectrum, `n_eff` (participation ratio of the **projected** spectrum), the cumulative curve `C(k)`, and per measurement `$moved_mass` (the absolute mass the truncation removed) and `$moved_share` (against total **absolute** mass, because the signed trace can be small, zero or negative while the form is far from PSD). Clipping is never silent: the same per-measurement numbers are on the projection receipt at `$projection` beside the operator that moved them, the execution receipt's `$scientific_plan_id` is derived from the source's and the projection's name and its `$task_partition_id` ends `+psd_projection`, and the print states the moved mass and the sentence "latent descriptive layer; not for inference". A measurement with no nonnegative mass is **masked**, not zeroed — `n_eff` and its whole `C(k)` row are `NA`, on the `coherence_fraction` template; `$moved_share` is the one quantity deliberately *not* masked when its denominator vanishes, because a form with no absolute mass moved none of it. The record is **sealed by re-derivation, not by range checks**: `n_eff` and `C(k)` are recomputed from `$spectrum` and must match bit for bit, the receipt's `clipped` / `masked` / `total_moved_mass` / `max_moved_share` are recomputed from the per-measurement series, and the identity `kept + moved = source absolute mass` is asserted at `1e-10` relative — so a record carrying the honest per-measurement mass under a `clipped: 0 of 4` headline, or zeroing the moved mass on object *and* receipt together, is a contract error rather than a plausible object. The signed source is byte-identical after construction (asserted, not assumed) and no fraction, cumulative curve or effective count was added to any signed view (`test-latent-geometry.R`). **Inherited, not resolved:** the masking guard is exact (`> 0`), the same guard every per-node fraction here uses, so a form that is zero only to within rounding is not masked and its `n_eff` is computed from the rounding — measured identical to a signal row's at `1e-9` on a `1e-17` fixture. That is G3's open tolerance decision (below), stated in the function's `Masking` section and characterized by a test rather than repaired privately | met |
...
| sampling covariance | `sampling_covariance(..., operation = "transport")` transports the covariance through an output-by-evidence linear map. **D8 added** the named `contrast_energy` route, `sampling_covariance(x, queries = )`: a bank of K centred contrasts is lowered onto the distance basis through the same packed operator `contrast_energy()` names (`svec(c c^T)`), and the K-by-K covariance returns as an `effect_sampling_covariance` with `basis = "query_bank"`, agreeing with the hand-built `"transport"` to 1e-19 on the D8 fixtures (`test-query-bank-sampling.R`). An uncentred contrast is refused: crossvalidated distances are invariant to `G -> G + a1' + 1a'` and an uncentred energy is not, so the distance basis carries no information about it | met per measurement. The **cross-node** covariance G8 asks for is still not supplied; `scope = "cross_measurement"` refuses by capability (`cross_node_sampling_covariance`) rather than returning per-node margins |
...
| 6 | contributions are signed; shares and clipping are invalid on the estimation layer | `tests/testthat/test-conservative-geometry-contract.R` — "contribution shares are undefined on the signed estimation layer"; `design/oracles/conservative-transport-readiness.R` §O3.e; `design/effect-form-contract.md` §8. **The other half — that the layer where they *are* valid exists and is labelled (D7)** — is `tests/testthat/test-latent-geometry.R`: "constructing the latent layer leaves the signed source untouched" asserts the signed packed rows and the signed spectrum are byte-identical after a latent layer is built, and "fractions and cumulative curves live only on the latent layer" asserts `cumulative` / `n_eff` / `moved_mass` / `moved_share` appear on the latent object and on no signed readout |
...
| G3 | "Coherent share versus location" is underdetermined when a location sits in several scales — it is a function of (location, scale), not a number | Define the spectrum on `(center, scale)` and require any location-wise collapse to be a declared, named reduction (α-weighted mean, argmax-scale, …). **Closed (D5):** `coherence_spectrum(x, weights, by_location = TRUE)` is that table, one row per `(scale, center)` with the key columns keeping their own types; the default `by_location = FALSE` groups by scale alone, and `by = ` takes any key tuple the metadata carries. No collapse is offered at all — a location's scale profile is a filter on the table, which needs no named operation, while an α-weighted mean or an argmax-scale map would need its own certificate and does not exist. `$metadata$aggregation$location_collapse` records `"none"` so a downstream reader can assert it. **One thing the gap did not anticipate:** a singleton scale has exactly zero configuration (§3.2, `effect-form-v1` §7), so its *aggregated* configuration sits within one ulp of zero and can land on either side of the §4 mask's `configuration >= 0` guard — the same coin flip a singleton node's own `coherence_fraction` already has. Measured on a 15-feature line fixture: exactly `0` for α ∈ {1, 1/3, 1/2}, `−2.8e−17` for α = 0.2, i.e. the contract's own headline "point scale reads φ = 1.000000" row reports `NA` for some weightings. D5 characterizes this rather than repairing it, because the guard is shared with every per-node fraction in the package; **owed:** a contract decision on whether the nonnegativity guard takes a relative tolerance | D5 ✔ |
...
| G7 | "Nonnegativity projection" is underdetermined — a per-node total clamp and an eigenvalue truncation of the form are different operators moving different mass | Latent layer takes a named projection from a closed set, and the receipt records moved mass per projection kind. **Closed (D7):** `latent_geometry()` (`R/latent.R`, §8). The set is `c("psd_projection", "nearest_psd")` with only the first built; the second refuses by capability `nearest_psd_projection` rather than falling back, which is the gap's own point — the two move different mass, so a silent substitution answers a different question. The per-node clamp the gap names as the *other* operator is not merely unimplemented but **refused**: a query readout (`contrast_energy()`, `rdm()`, `rsa()`, `crossnobis()`, a `query_geometry()` view) has already been contracted against fixed weights, so `latent_geometry()` signals `latent_projection_source` on it and names the clamp as the different projection it would be. Moved mass is recorded per measurement, per projection kind, on the `effect_latent_projection_receipt` at `$projection`; the *execution* receipt was not widened to hold it, because its canonical field list is shared by every receipt in the package and a latent-only field would sit empty on all of them — the projection instead enters the two receipt fields that already have room for it, the derived `$scientific_plan_id` and the `+psd_projection` suffix on `$task_partition_id`. **One thing the gap did not anticipate:** a measurement can have *no* nonnegative mass at all (a negative-definite form), where the projection's own functionals are `0/0`. `n_eff` and `C(k)` are masked there rather than reported as zero, and `$projection$masked` counts them — the same withholding `coherence_fraction` does, and the reason the layer object cannot be replaced by `pmax(spectrum, 0)`. **Owed, and now owed more sharply:** that guard is exact rather than relative, so G3's open tolerance question now governs three more functionals than it did — a form that is zero to within rounding gets an `n_eff` and a `C(k)` computed from the rounding, identical to a signal row's because the ratio is scale free. D7 inherits the decision rather than making a private one, documents the consequence, and pins it with a test that has to change the day G3 is decided. **Owed to WS-E:** `design/population-form-contract.md` §9.1's `latent_projection_named` readiness row can now be supplied, and is still recorded there as missing | D7 ✔ |
| G8 | Conservation gives no uncertainty; nothing currently supplies cross-node sampling covariance for `contrast_energy` (§7.6a) | Named `contrast_energy` sampling route returning the cross-node covariance, not per-node margins. **Half closed (D8):** the named route exists — `sampling_covariance(x, queries = )` returns the K-by-K analytic covariance of a bank of contrast energies, per measurement, in a `basis = "query_bank"` form, and is the exact transport of the distance-basis covariance through the packed query `contrast_energy()` names. The cross-node half is **not** closed and was not closeable from this machinery: the admitted law reads one measurement's own support, residual covariance, and signal patterns, and nothing in `evidence-sampling-v1` supplies the cross-support residual second moment or the frame overlap that a joint covariance needs. Rather than return margins that could be mistaken for a joint block, `scope = "cross_measurement"` refuses with capability `cross_node_sampling_covariance`, naming both what is missing and §7.6a's reason that a conserved ledger does not supply it. **Still owed:** an explicit spatial covariance model, which would be a new admission in the sampling contract, not a new query route. Until then `precision_weighted` (population contract §14.1) stays gated on an external precision source | D8 (partial) |
