...
**97 + 13 = 110**, which is what `tests/testthat/test-api-surface.R` pins. A
...
| `population_uncertainty()` | `R/population-uncertainty.R` | advanced (experimental: population) | ws-e (ticket E8) | the reader verb for the two error bars `design/population-form-contract.md` §7 requires to be reported apart, and the only place either is computed. It is a separate export rather than a field on the result because the between-subject layer is *derived* — it is a slice-wise function of `$residuals`, `$residual_df` and one `p×p` unscaled covariance the fit records, so storing it would add three `node×query×term` arrays beside the coefficients for a number a reader may never ask for — and because a verb is where the two refusals live: a `complete_form` result retains no participant residuals by design (`population_between_subject_residuals`), and a saturated group model has no residual scatter to estimate (`between_subject_residual_df`), both of which a passive field would have had to express as `NA`. What it adds over `summary(lm())` at the call site is the discipline that a hand-rolled group test drops. The two layers are **structurally** unpoolable: `$between` and `$within` are separate blocks, `as.data.frame()` takes a `layer` argument and emits one at a time, and there is no field holding their sum — because the between-subject residual already contains whatever measurement error survived into each participant's transported value, so adding the within-subject variance double-counts the shared part and still misses the covariance a variance-components model would need. The within layer is admitted **only** where it is exact: a group column fed by exactly one native row, where the cross-node terms carry weight zero, per participant and per column, with `$within$admitted` recording which. No independence assumption is made anywhere and none would be defensible — overlapping searchlight supports under spatially correlated noise are positively correlated, so a diagonal sum over a multi-source column would understate the variance in a known direction, which is a worse failure than the absence D8 already refuses under `cross_node_sampling_covariance`. And the `t` is labelled `uncalibrated` in every field, on every printed line and in the Rd, held there by measurement rather than by caution: `benchmarks/run-population-null-coverage.R` records nominal coverage (0.9485 to 0.9520 over N = 6, 8, 12, 24) under a correctly specified group model and 0.9230 to 0.8850 under covariate-linked heteroskedasticity — an 11.5 % false positive rate at N = 24 against a nominal 5 %, degrading as the study *grows*. Users: T:1 (`test-population-uncertainty.R`) | keep |

| `heterogeneity()` | `R/population-heterogeneity.R` | advanced (experimental: population) | ws-e (ticket E7) | the reader for the *other* half of `design/population-form-contract.md` §6's decomposition. `estimate_population()` and `materialize_population()` compute the consensus term `Q^C`; nothing computed `Q^H`, and there was no route to one that a user could write correctly at the call site. §5 is the reason: geometry-space covariance across `N` participants has rank at most `N-1`, so its whole spectrum lives in an `N`-by-`N` Gram of `svec`'d transported forms and the `D`-by-`D` covariance never has to be formed (measured: spectra agree to `1.07e-14`, mode recovery `|<v1, first eigenvector>| = 1.000000000000000`, and the Gram is node-additive to `3.55e-15`, so it accumulates while streaming). Written by hand that becomes `cov()` of an `N`-by-`m`-by-`p` stack, which is the array §5 exists to say must never be built. Three things are refusals here rather than caveats. First, the packing: the Euclidean inner product of packed rows is the Frobenius inner product of forms *only* with the codec's `sqrt(2)` off-diagonals, and a naive `upper.tri` packing is off by `2.96e+00` relative — an `O(1)` error, not a tolerance — so a spectrum computed from it is not a spectrum of geometries; the receipt names the codec and a test breaks if it changes. Second, the estimator: §6.2 measures the plug-in split booking every participant's own within-subject sampling noise as between-participant heterogeneity, inflating the heterogeneity trace by **+62.7 %**, and §6.4 is normative that a population plan computes the cross-fitted Gram, records which partitions formed the two halves, and refuses to report any spectrum, variance-explained figure or `n_eff` from the plug-in one. So `estimator` is a closed set of two, it enters the record's identity, `"cross_fit"` is the default and re-plans every participant over two disjoint halves of their partitions (four partitions is the floor, and fewer is a named refusal rather than a silent fallback), and the plug-in route ships with `$latent` `NULL` and the refusal on the receipt — its loading *directions* stay admissible, named as such, which is exactly what §6.3 licenses. Third, the latent discipline: §6.5 is `conservative-geometry-v1` §6 one level up, so `n_eff`, the cumulative curve and every fraction live on a named PSD projection whose moved mass is recorded, and they are built with `latent_geometry()`'s own operators rather than a second copy of them. The sink is excluded from the Gram and the exclusion is recorded: it is unmapped territory in budget units under either semantics, so an inner product including it would add a mass to a density. Users: T:1 (`test-population-heterogeneity.R`) | keep |

Tier arithmetic after this section: core 21 → **23**; advanced 33 → **35**;
advanced (experimental) 16 → **25**, of which the nine new ones are the
population sub-label; total 97 → **110**.
