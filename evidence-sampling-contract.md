...
- \(\Delta\) contains the true distance cross-products;
...
2. Eq. 13 against an independent direct or closed-form oracle;
3. the complete \(D\times D\) covariance, not only its diagonal;
4. exported product-path agreement with an oracle importing no package
   internals;
5. null centering and signal-dependent variance in generative simulations;
6. coverage under the declared plug-in or null policy;
7. refusal when the error channel is absent;
8. refusal or qualification for learned metric uncertainty;
9. unequal-partition negative fixtures;
10. direct-query versus full-materialization equivalence; and
11. runtime and peak-memory gates at realistic \(q\), frame, and feature sizes.
