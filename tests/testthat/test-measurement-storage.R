test_that("memory measurement storage round-trips every logical block", {
  ...
  plan <- crossform:::.measurement_result_plan(fixture$task)
  store <- crossform:::.memory_measurement_store(
  ...
  for (edge in seq_along(run$blocks)) {
    expect_equal(
      crossform:::.read_measurement_store(store, edge),
      run$blocks[[edge]], tolerance = 0
    )
    expect_equal(
      crossform:::.read_measurement_store(store, names(run$blocks)[[edge]]),
      run$blocks[[edge]], tolerance = 0
    )
  }
})
...
test_that("block-backed measurement storage is complete only after every write", {
  ...
  store <- crossform:::.file_measurement_store(
  ...
  expect_silent(crossform:::.validate_measurement_store(written))
  expect_identical(file.info(path)$size,
    sum(plan$block_index$length_elements) * 8)
  for (edge in seq_along(run$blocks)) {
    expect_equal(
      crossform:::.read_measurement_store(written, edge),
      run$blocks[[edge]], tolerance = 0
    )
  }
})
