test_that("measurement legs bind source, output, basis, units, and support", {
  domain <- abstract_domain(3, feature_ids = c("v1", "v2", "v3"),
    id = "neural:v1")
  output <- effectagram:::.measurement_axis(
  ...
  leg <- effectagram:::.measurement_leg(
  ...
  expect_silent(effectagram:::.validate_measurement_leg(leg, domain))

  forged <- leg
  forged$support <- c("v1", "v2")
  expect_error(effectagram:::.validate_measurement_leg(forged), "support")
})

test_that("equal metrics do not erase oriented measurement identity", {
  domain <- abstract_domain(3, id = "neural:v1")
  first_axis <- effectagram:::.measurement_axis(
    c("a", "b"), "roi:v1", basis_id = "basis:original"
  )
  rotated_axis <- effectagram:::.measurement_axis(
    c("u", "v"), "roi:v1", basis_id = "basis:rotated"
  )
  first_operator <- matrix(c(1, 2, 0, 0, 1, 3), 2, 3, byrow = TRUE)
  rotation <- matrix(c(0, -1, 1, 0), 2, 2)
  first <- effectagram:::.measurement_leg(
    first_operator, domain, first_axis
  )
  rotated <- effectagram:::.measurement_leg(
    rotation %*% first_operator, domain, rotated_axis
  )

  expect_equal(
    effectagram:::.measurement_leg_metric(first),
    effectagram:::.measurement_leg_metric(rotated),
    tolerance = 1e-14
  )
...
test_that("learned legs require frozen portable training provenance", {
  ...
  output <- effectagram:::.measurement_axis("mode", "learned:mode")
  training <- paste0("sha256:", paste(rep("a", 64), collapse = ""))

  expect_error(effectagram:::.measurement_leg(
    matrix(c(1, 2), 1), domain, output, estimation = "learned_frozen"
  ), "frozen training provenance")
  learned <- effectagram:::.measurement_leg(
    matrix(c(1, 2), 1), domain, output, estimation = "learned_frozen",
    provenance = list(
      frozen = TRUE,
  ...
  expect_silent(effectagram:::.validate_measurement_leg(learned))
  expect_error(effectagram:::.measurement_leg(
    matrix(c(1, 2), 1), domain, output, estimation = "learned_frozen",
...
test_that("direct sums preserve named component identities and ranges", {
  domain <- abstract_domain(3, id = "neural:v1")
  coherent <- effectagram:::.measurement_leg(
    matrix(c(1, 1, 1) / sqrt(3), 1), domain,
    effectagram:::.measurement_axis("mean", "coherent:v1")
  )
  configuration <- effectagram:::.measurement_leg(
...
test_that("measurement frames keep ordered legs and conservative dual metadata", {
  ...
  frame <- effectagram:::.measurement_frame(
  ...
  rank_deficient <- effectagram:::.measurement_frame(legs[1:2])
  expect_false(rank_deficient$injectivity$guaranteed)
  expect_false(rank_deficient$dual$eligible)
  expect_error(effectagram:::.measurement_frame(
...
test_that("measurement edges contain only explicit ordered requests", {
  ...
  frame <- effectagram:::.measurement_frame(stats::setNames(
    lapply(1:3, make_leg), c("a", "b", "c")
  ))
  edges <- effectagram:::.measurement_edges(
  ...
  reversed <- effectagram:::.reverse_measurement_edges(edges, frame)
  expect_identical(reversed$edges$left, edges$edges$right)
  expect_identical(reversed$edges$right, edges$edges$left)
  expect_identical(
    effectagram:::.reverse_measurement_edges(reversed, frame), edges
  )
  expect_error(effectagram:::.measurement_edges(
    c("a", "a"), c("b", "b"), frame
  ), "unique explicit")
})
...
test_that("square-root additive legs exactly recover every local geometry", {
  ...
  for (normalization in c("none", "local", "conservative")) {
    ...
    measured <- effectagram:::.measurement_frame_from_additive(frame, "total")
...
test_that("coherent common-mode legs recover the certified coherent component", {
  ...
  coherent <- effectagram:::.measurement_frame_from_additive(frame, "coherent")
...
test_that("additive adapters are canonical across dense and sparse storage", {
  ...
  sparse <- additive_frame(Matrix::Matrix(weights, sparse = TRUE),
    domain = domain)
  dense_frame <- effectagram:::.measurement_frame_from_additive(dense)
  sparse_frame <- effectagram:::.measurement_frame_from_additive(sparse)
...
test_that("additive sparse identity is canonical without densification", {
  ...
  expect_identical(
    effectagram:::.additive_frame_signature(sparse_zero),
    effectagram:::.additive_frame_signature(sparse)
  )
  expect_identical(
    effectagram:::.additive_frame_signature(dense),
    effectagram:::.additive_frame_signature(sparse)
  )
  expect_s4_class(
    effectagram:::.canonical_additive_weights(stored_zero),
    "CsparseMatrix"
  )
  changed <- canonical
  ...
  expect_false(identical(
    effectagram:::.additive_frame_signature(sparse),
    effectagram:::.additive_frame_signature(
